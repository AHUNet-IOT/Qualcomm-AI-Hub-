# Qualcomm-AI-Hub-
Qualcomm AI Hub 端到端模型部署教程：支持任意 PyTorch 模型云端编译、NPU 性能分析、云端推理验证，并导出可直接部署的 DLC 模型。包含完整环境配置、模型保存（torch.jit.trace）、代理设置、常见问题解决方案与可复用脚本。适用于不同芯片与输入维度，新手可一键上手。

> 适用于任意 PyTorch 模型云端编译、NPU 性能测试、推理验证，并导出可部署到各类高通芯片的 DLC 模型

> 注：教程中以 SA8295P ADP 芯片、38维输入为例，可根据自身需求替换为任意芯片与输入维度
...
# 2. 生成与实际输入规格一致的模拟输入（示例为 (1,1,38)，可替换为你的输入维度）
dummy_input = torch.randn(1, 1, 38)  # 替换为你的输入形状（如 (1,3,224,224)）
...
4. 在真实 NPU 硬件上执行性能 profiling（延迟、内存测试）
